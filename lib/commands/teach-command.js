 ...
 * @constructor
 */
var TeachCommand = function(commandChain, botName, providerPool) {
  ...
  this.help    = helpBuilder.build('teach', botName,
                 '\tteach a new commands to ' + botName + ', usage:\n'
  ...
  this.tokenizers = [
    ...
    { 'id': 'inject', 'regexp': /\$[a-z.]+/g,
      'do': function(token, message, translations) {
        //self.providerPool.generateHandlers(tokens);

        var user;

        if (token.indexOf('$self') === 0) {
          user = self.providerPool.get('flow').getUserById(message.user);

          return !user ? token : util.objectAccess(user, token);
        }

        if (token.indexOf('$user') === 0 && translations.hasOwnProperty('$user')) {
          user = self.providerPool.get('flow').getUserByNick(translations['$user']);
...
TeachCommand.prototype.exec = function(message) {
  ...
  var author = this.providerPool.get('flow').getUserById(message.user);
  ...
  this.commandChain.add(new Command({
    'name':       name,
    'matcher':    matcher,
    'action':     action,
    'paramsMap':  paramsMap,
